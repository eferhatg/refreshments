...
export default class BinarySearchTree {
  ...
  containsRecursive(val,node){
    ...
    if(val<node.data&&node.left) return this.containsRecursive(val,node.left);
    if(val>=node.data&&node.right) return this.containsRecursive(val,node.right);
    return false;
  }
  ...
  // validateRecursive(node,min=null,max=null){
  ...
  countLeaves(){
    ...
    var leaveCount=0;
    var traverse=(node)=>{
      if(!node.left&&!node.right)leaveCount++;
